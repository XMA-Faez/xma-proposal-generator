-- Add sales rep features to the database (Safe version)
...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE profiles ADD CONSTRAINT valid_role CHECK (role IN ('admin', 'sales_rep'));
    END IF;
END $$;
...
-- Make created_by NOT NULL after backfilling (only if column exists and is nullable)
DO $$
BEGIN
    IF EXISTS (
...
-- RLS Policies for activity_logs (drop and recreate to avoid conflicts)
DROP POLICY IF EXISTS "Admins can view all activity logs" ON activity_logs;
DROP POLICY IF EXISTS "Sales reps can view own activity logs" ON activity_logs;
...
DROP POLICY IF EXISTS "Admins can insert proposals" ON proposals;
DROP POLICY IF EXISTS "Admins can update all proposals" ON proposals;
...
DROP POLICY IF EXISTS "Sales reps can insert proposals" ON proposals;
