...
interface GeneratorSummaryProps {
  ...
  discounts: {
    ...
    serviceDiscounts: Record<string, { type: 'percentage' | 'absolute', value: number }>;
...
const GeneratorSummary: React.FC<GeneratorSummaryProps> = ({
    ...
    if (includePackage && selectedPackage) {
      ...
      if (discounts.packageDiscount.value > 0) {
        if (discounts.packageDiscount.type === 'percentage') {
          packagePrice = packagePrice * (1 - discounts.packageDiscount.value / 100);
        } else {
          packagePrice = Math.max(0, packagePrice - discounts.packageDiscount.value);
        }
      }
  ...
  const monthlyFees = selectedServices
    .filter(service => service.is_monthly && service.setup_fee)
    .reduce((total, service) => total + service.setup_fee, 0);
            ...
            {includePackage && selectedPackage && (
              <div className="grid grid-cols-1 md:grid-cols-12 gap-2 pb-3 border-b border-zinc-800">
                <div className="md:col-span-4">
                  <span className="text-sm text-zinc-400">{selectedPackage.name} Package Discount:</span>
                </div>
                <div className="md:col-span-8">
                  <DiscountControl
                    ...
                    onChange={(value, type) => onDiscountChange('package', null, value, type)}
                    className="mb-0"
                  />
                </div>
              </div>
            ...
            {selectedServices.length > 0 && (
              <div className="space-y-3">
                {selectedServices.map((service) => {
                  ...
                  return (
                    <div key={service.id} className="grid grid-cols-1 md:grid-cols-12 gap-2 pb-3 border-b border-zinc-800">
                      <div className="md:col-span-4">
                        <span className="text-sm text-zinc-400">{service.name} Discount:</span>
                      </div>
                      <div className="md:col-span-8">
                        <DiscountControl
                          ...
                          onChange={(value, type) => onDiscountChange('service', service.id, value, type)}
                          className="mb-0"
                        />
                      </div>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-2 pt-3">
              ...
              <div className="md:col-span-8">
                <DiscountControl
                  ...
                  maxValue={subtotal}
                  onChange={(value, type) => onDiscountChange('overall', null, value, type)}
                  className="mb-0"
                />
              </div>
            </div>
      ...
      <div className="bg-zinc-900 p-4 rounded-lg">
        <div className="space-y-2 mb-4">
                  ...
                  {formatPrice(
                    discounts.packageDiscount.value > 0 
                      ? (discounts.packageDiscount.type === 'percentage'
                          ? selectedPackage.price * (1 - discounts.packageDiscount.value / 100)
                          : selectedPackage.price - discounts.packageDiscount.value)
          ...
          {selectedServices.length > 0 && selectedServices.map((service) => {
            const serviceDiscount = discounts.serviceDiscounts[service.id];
            const discountedPrice = serviceDiscount && serviceDiscount.value > 0
            ...
            return (
              <div key={service.id} className="flex justify-between items-center text-sm">
                ...
                <div className="text-right">
                  ...
                  <span>{formatPrice(discountedPrice)} {service.currency}</span>
                </div>
              </div>
            );
          })}
